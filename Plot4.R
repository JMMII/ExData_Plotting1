...
Plot4Data <- transform(projData, DateTime = strptime(paste(projData$Date, substr(projData$Time,12,16)),format = "%Y-%m-%d %H:%M"))

#open graphics device, create filename and set width and height for Plot 4
png(filename = "Plot4.png", width = 480, height = 480)

#set plotting parameter to a 2x2 plot, set point size and margin size. Plots will be created by columns  
par(mfrow=c(2,2), ps=12, mar=c(4,4,1,1), mgp=c(2,0.5,0))

#create top-left plot 
with(Plot4Data,plot(DateTime, Global_active_power, type = "l", ylab = "Global Active Power", xlab = "", col = "black"))

#create top-right plot
with(Plot4Data,plot(DateTime, Voltage, type = "l", ylab = "Voltage", xlab = "datetime", col = "black"))

#create bottom-left plot (same as plot 3 from the project)
#open plot and make first layer
with(Plot4Data, plot(DateTime, Sub_metering_1, type = "l", xlab = "", ylab = "Energy sub metering"))

#second layer
lines(Plot4Data$DateTime, Plot4Data$Sub_metering_2, type = "l", col = "red")

#third layer
lines(Plot4Data$DateTime, Plot4Data$Sub_metering_3, type = "l", col = "blue")

#create the legend
legend("topright", legend = c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"), col = c("black", "red", "blue"), lwd = 1, seg.len = 1, bty="n", y.intersp = .75)

#create bottom right plot
with(Plot4Data,plot(DateTime, Global_reactive_power, type = "l", ylab = "Global_reactive_power", xlab = "datetime", col = "black", yaxt = "n"))
axis(2, at = seq(0.0,0.5, by = 0.1), las=2)

#turn off the graphics device for Plot 4
dev.off()
